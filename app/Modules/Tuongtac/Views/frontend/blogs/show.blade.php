            ...
            <div class="action-buttons" style="position: absolute; top: -0px; right: 4px;z-index:1000">
                ...
                @if ($post->status == 1)
                    <a href="{{ route('front.tblogs.status', $post->id) }}" title=" ẩn bài viết " style=" "><button
                            class="deletebtn"> <i class="fa fa-times-circle  " style="background:white"></i> </button></a>
                @else
        ...
        <div class="post-item">
            ...
            @if ($images)
                <div class="post-image">
                    <img src="{{ $images ? $images[0] : '' }}" onclick=" openLightbox('{{ $json_photo }}', 0)"
                        alt="{{ $post->title }}">
                </div>
                <div class="thumbnail-container">
                    @foreach ($images as $image)
                        @if ($image != '')
                            <img class="thumbnail" onclick=" openLightbox('{{ $json_photo }}', {{ $loop->index }})"
                                src="{{ $image }}" alt="Hình ảnh rao vặt">
                        @else
            ...
            <div class="post-author">
                ...
                <div class="author-info">
                    <a href="{{ $post->user_url }}">
                        <h3>{{ $post->author ? $post->author->full_name : 'Người dùng không xác định' }}</h3>
                    </a>
