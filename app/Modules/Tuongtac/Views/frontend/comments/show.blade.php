 ...
 <div class="comments-section">
     ...
     <div class="comment-form">
         <textarea id="te-{{ $item_id }}" class="comment-textarea" placeholder="Viết bình luận của bạn..."></textarea>
             ...
             <div id='acomment-{{ $item_id }}-{{ $comment->id }}' class="comment">
                <div class="comment-user">
                 ...
                 <div class="reply-form" id="reply-form-{{ $comment->id }}" style="display: none;">
                     <textarea id="te-{{ $item_id }}-{{ $comment->id }}" class="comment-textarea" placeholder="Phản hồi lại..."></textarea>
                     ...
                     @foreach ($comment->subcomments as $subcomment)
                         ...
                         <div id='acomment-{{ $item_id }}-{{ $subcomment->id }}' class="replies">
                             ...
                             <div class="reply-form" id="reply-form-{{ $subcomment->id }}" style="display: none;">
                                 <textarea id="tes-{{ $item_id }}-{{ $comment->id }}-{{ $subcomment->id }}" class="comment-textarea" placeholder="Phản hồi lại..."></textarea>
 ...
 <script>
     ...
     if (typeof submitComment !== 'function') {
         function submitComment(item_id) {
             ...
             const dataToSend = {
                 ...
                 item_id: item_id,
                 item_code: item_code,
                 book_mentions: {} // Thêm object chứa thông tin các mentions
             };
             
             // Xử lý mentions
             const mentionsData = document.getElementById('book-mentions-data');
             if (mentionsData) {
                 // Tìm tất cả mentions trong textarea
                 const mentionRegex = /@[^[]+\[mention-([^\]]+)\]/g;
                 let match;
                 while ((match = mentionRegex.exec(content)) !== null) {
                     const fullMatch = match[0];
                     const idPart = fullMatch.match(/\[mention-([^\]]+)\]/);
                     if (idPart && idPart[1]) {
                         const mentionId = 'mention-' + idPart[1];
                         const mentionDataAttr = mentionsData.getAttribute(`data-${mentionId}`);
                         if (mentionDataAttr) {
                             try {
                                 const mentionData = JSON.parse(mentionDataAttr);
                                 dataToSend.book_mentions[mentionId] = mentionData.bookUrl;
                             } catch (e) {
                                 console.error('Error parsing mention data:', e);
                             }
                         }
                     }
                 }
     ...
     if (typeof addReplyToDOM !== 'function') {
         function addReplyToDOM(item_id, subcomment, comment_id) {
             ...
             <div class="replies">
                ...
                <div class="reply-form" id="reply-form-${subcomment.id}" style="display: none;">
                    <textarea id="tes-${item_id}-${comment_id}-${subcomment.id}" class="comment-textarea" placeholder="Phản hồi lại..."></textarea>
     ...
     if (typeof submitReply !== 'function') {
         function submitReply(item_id, comment_id) {
             // Xử lý gửi phản hồi cho bình luận
             const content = document.getElementById(`te-${item_id}-${comment_id}`).value;
             if (content.trim() === '') {
             ...
             $.ajax({
                 ...
                 success: function(response) {
                     if (response.status) {
                         document.getElementById(`te-${item_id}-${comment_id}`).value = "";
                         document.getElementById(`reply-form-${comment_id}`).style.display = "none";
                         addReplyToDOM(item_id, comment_id, response.msg);
                     } else {
                         alert(response.msg);
                     }
                 },
