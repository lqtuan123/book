...
class TCommentController extends Controller
{
    ...
    public function saveComment(Request $request)
    {
        ...
        \Log::info('Original comment content: ' . $content);
        
        // Tìm các mention theo pattern mới: @TênSách[mention-id]
        preg_match_all('/@([^[]+)\[mention-([^\]]+)\]/', $content, $matches, PREG_SET_ORDER);
        
        foreach ($matches as $match) {
            $fullMatch = $match[0]; // @TênSách[mention-id]
            $bookTitle = $match[1]; // TênSách
            $mentionId = 'mention-' . $match[2]; // mention-id
            
            // Tìm URL của sách trong dữ liệu gửi lên (nếu có)
            $bookUrl = '';
            if (isset($request->book_mentions) && isset($request->book_mentions[$mentionId])) {
                $bookUrl = $request->book_mentions[$mentionId];
            } else {
                // Tìm sách trong database
                $book = \App\Modules\Book\Models\Book::where('title', 'like', '%' . trim($bookTitle) . '%')
                    ->where('status', 'active')
                    ->first();
                
                if ($book) {
                    $bookUrl = route('front.book.show', $book->slug);
                }
            }
            
            if (!empty($bookUrl)) {
                // Thay thế mention bằng HTML đã được xử lý
                $replacementHtml = '<a href="' . $bookUrl . '" class="book-mention">@' . $bookTitle . '</a>';
                $content = str_replace($fullMatch, $replacementHtml, $content);
                \Log::info('Replaced mention: ' . $fullMatch . ' with: ' . $replacementHtml);
            }
        }

        // Cập nhật nội dung đã xử lý vào data
        $data['content'] = $content;
        \Log::info('Processed comment content: ' . $content);
